...
class Application(ctk.CTk):
    def __init__(self, parse):
        ...
        persistence.load_files(
            left=self.left_frame_init.llistbox, right=self.right_frame_init.rlistbox
        )
...
class left_frame(tk.Frame):
    def __init__(self, parent):
        ...
        self.label_left = tk.Label(
            self, textvariable=parent.upload_path, width=20, height=2, anchor="w"
        )
        ...
        self.llistbox_but = tk.Button(
            self, text="Select", command=lambda: utils.select_button()
        )
        ...
        self.lbutton = tk.Button(
            self,
            text="Open Files",
            command=lambda: utils.select(parent.upload_path, self.llistbox),
        )
...
class right_frame(tk.Frame):
    def __init__(self, parent):
        ...
        self.label_right = tk.Label(
            self, textvariable=parent.deload_path, width=20, height=2, anchor="w"
        )
        ...
        self.rbutton = tk.Button(
            self,
            text="Open Folder",
            command=lambda: utils.select(parent.deload_path, self.rlistbox),
        )
...
class middle_frame(tk.Frame):
    def __init__(self, parent, parse):
        ...
        omni = tk.Radiobutton(
            self, text="OMNI", variable=parent.parse_form, value="OMNI"
        )
        ...
        process = tk.Button(
            self,
            text="Parse",
            command=lambda: utilities.handle_files(
